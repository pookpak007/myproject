...
<div class="form-group {{ $errors->has('checking') ? 'has-error' : ''}}">
    <label for="checking" class="control-label">{{ 'Checking' }}</label>
    <input class="form-control" name="checking" type="text" id="checking" value="{{ isset($order->checking) ? $order->checking : ''}}" >
    {!! $errors->first('checking', '<p class="help-block">:message</p>') !!}
</div>
